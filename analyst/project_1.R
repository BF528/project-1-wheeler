...
bio_combat_welch_results <- bio_combat_welch_res %>% mutate(PROBEID = rownames(.)) %>% 
...
gene_list_no_duplicates <- gene_symbols[!duplicated(gene_symbols$SYMBOL),] %>% 
  left_join(bio_combat_welch_results)
...
write.csv(bio_combat_welch_up_regulated, file="/projectnb/bf528/users/wheeler/project_1/analysis/analyst/top_10_up_regulated.csv", row.names=F)
...
write.csv(bio_combat_welch_down_regulated, file="/projectnb/bf528/users/wheeler/project_1/analysis/analyst/top_10_down_regulated.csv", row.names=F)
...
## part 6.5 -  a function to do fisher test ##
